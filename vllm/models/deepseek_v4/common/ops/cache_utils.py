...
def quantize_and_insert_k_kernel(
    ...
    ``use_fnuz=True`` selects FNUZ (``tl.float8e4b8``); default OCP e4m3fn
    (hardware convert on SM89+, software encode on SM80) matches every
    production caller.
    """
    pid = tl.program_id(0)
    ...
    for qblock_idx in tl.static_range(n_quant_blocks):
        ...
        if qblock_start < fp8_dim:
            ...
            # Convert to fp8 (FNUZ on gfx942, software e4m3fn on SM80).
    ...
    for i in range(worker_id, gather_len, num_workers):
        ...
        for qblock_idx in tl.static_range(n_quant_blocks):
            ...
            if qblock_start < fp8_dim:
                ...
                # Decode fp8 bytes (FNUZ on gfx942, software e4m3fn on SM80).
